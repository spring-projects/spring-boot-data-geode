...
backed by Apache Geode as the caching provider in a Inline Caching use case.
...
Additionally, this example builds on the concepts from the link:caching-look-aside.adoc guide.  Therefore, it would be
helpful to have read that guide before proceeding through this guide.
...
Consider a very simple mathematical computation, the _factorial_.  A _factorial_ is defined as `factorial(n) = n!`.
...
While _Inline Caching_ can take several forms, the form of _Inline Caching_ we present here will be an extension to
the _Look-Aside Cache_ pattern.  This particular form of _Inline Caching_ is useful in cases where:
...
3. Multiple, discrete and disparate applications are sharing the same data source (contrary to Microservices design).
...
This form of _Inline Caching_ is very basic since 1) most application developers are not directly interfacing with
_Spring's Cache Abstraction_ in their application by using the `org.springframework.cache.Cache` API.  Most of the time,
application developers will use the Spring cache annotations (e.g. `@Cacheable`), or alternatively the JCache (JSR-107)
annotations, as {spring-framework-docs}/integration.html#cache-jsr-107[documented]. 2) Second, while the
`Cache.get(:Object, :Callable)` satisfies _read-through_ to the backend, external data source, there is no equivalent
operation in the `Cache` API for _write-through_, i.e. when using `Cache.put(key, value)` to put a value into the cache
in addition to the external data source.
...
(e.g. another application not using _Inline Caching_, thus not using a synchronous _write-through_ and instead
updating the database directly).  To keep the cache and database consistent, then all data access operations must
involve the cache.  That is, you must strictly adhere to and be diligent in your use of _Inline Caching_.
...
Both can occur in a single operation, on read.

When a client sends a request for data (#6) the request invokes the appropriate application (`@Cacheable`) service method,
which is immediately forwarded to the cache to determine if the results for the given input have already been computed (#5).
If the result is cached (a _cache hit_), then the result is simply returned to the caller.  However, if a result had not
been previously computed, or the result expired, or was evicted, then before the cacheable service method is invoked,
an additional lookup is performed (#4) to determine whether the computed value may have already been persisted to
the backend database.  If the value exists in the database, then it is loaded into the cache and returned to the caller.
Only when the computed value is not present in the cache nor exists in the database is the cacheable service method invoked.
Once the service method finishes and returns the result, the value is cached as part of the contract of `@Cacheable`
and will also be written through to the backend database.
...
For our example, we will develop a calculator service that performs basic mathematical functions, such as
...
The `CalculatorService` is annotated with Spring's `@Service` stereotype annotation so it is picked up by
...
expired or been evicted.  We neither configure eviction nor expiration for this example.
...
It is worth noting, however, that we are starting with the same applied pattern for caching as you would when using
the _Look-Aside Caching_ pattern.  This is key to minimizing the invasive nature of _Inline Caching_.  There is
a subtle difference, though, and that will be apparent in the additional configuration we supply as part of our
...
mechanism for "_read-through_" behavior as well as configure the same SD _Repository_ for a Region's `CacheWriter`
mechanism for "_write-through_" behavior.  This "_read/write-through_" behavior is the "inlining" component of
_Inline Caching_, i.e. the second lookup opportunity we talked about in the
<<geode-samples-caching-inline-background,Background>> section above.
...
data stores and 3) has a very consistent and intuitive API, following the https://www.baeldung.com/java-dao-pattern[Data Access Object (DAO) pattern]
...
In our case, we only want to target the Regions that have been used as "caches" identified in the service methods
...
For example, you may have a cache Region X containing data that needs to be stored in MongoDB (then simply use
https://spring.io/projects/spring-data-mongodb[Spring Data MongoDB's] Repository support), where as another
cache Region Y may contain data that needs to be written to Neo4j and represented as a graph (cool, then simply use
https://spring.io/projects/spring-data-neo4j[Spring Data Neo4j's] Repository support), and yet another cache Region Z
containing data that needs to be written back to a database (no problem, use https://spring.io/projects/spring-data-jdbc[Spring Data JDBC]
...
API (i.e. programming model) across a disparate grouping of data stores.  And due to that uniformity, it is easy to
"adapt" the GemFire/Geode `CacheLoader`/`CacheWriter` interfaces to use a SD Repository under-the-hood.  Indeed,
...
Also notice the use of the `@EnableCachingDefinedRegions` annotation.  Whenever you use a caching provider like
Apache Geode or Redis, you must explicitly define or declare your caches in some manner.  The inconvenience of this is,
you have basically already done so by declaring the caches required by your application when using Spring's,
or alternatively, the JSR-107, JCache API annotations (e.g. `@Cacheable`).  Why should you have to do this again?

Well, with SBDG, you don't.  You simply have to declare the `@EnableCachingDefinedRegions` annotation and SBDG
will take care of defining the Apache Geode Regions backing the caches for you.

This is not auto-configured for you because there are many ways to "define" a Region, with different eviction
and expiration polices, different memory requirements, application callbacks, etc.  The Region may already exist
and have been created some other way.  Either way, you may not want SBDG to auto-configure these Regions for you.

TIP: If you have not done so already, you should definitely read about SBDG's support for _Inline Caching_ in the
<<geode-caching-provider-inline-caching,documentation>>.

TIP: To learn more about how Apache Geode's (or Pivotal GemFire's) data loading mechanics work, or specifically, how to
{apache-geode-docs}/developing/outside_data_sources/sync_outside_data.html["Keep the Cache in Sync with Outside
...
{spring-data-geode-docs-html}/#bootstrap-annotation-config-caching[documentation].
...
With MongoDB or Redis, again you would configure the data source, or connection factory, appropriately and plug that
into the data access API of choice (e.g. Spring Data MongoDB or Spring Data Redis).

Though it is not immediately apparent, in our example, we simply 1) used an embedded databased (i.e. HSQLDB)
and 2) relied on Spring Boot's _auto-configuration_ to bootstrap the embedded database on startup.

TIP: To learn more about {spring-boot-docs-html}/boot-features-sql.html#boot-features-embedded-database-support[bootstrapping]
...
With these dependencies declared on the application classpath, Spring Boot auto-configures a `DataSource` to
an embedded HSQLDB database, which bootstraps, finds our application `CalculatorRepository` interface declaration,
and backs it with a Spring Data JPA implementation using Hibernate as the provider.  Very powerful!

Additionally, we configure the embedded, in-memory HSQLDB database by including a SQL script with a DDL statement
to initialize the schema (i.e. create the "CALCULATIONS" table):
...
As well as include a SQL script with DML to to pre-populate the database with some existing data
...
Simply by including the `schema.sql` and complimentary `data.sql` files in the classpath of the application, Spring Boot
...
{spring-boot-docs-html}/howto-database-initialization.html[here].
...
the data model).  There is not a whole lot of difference (i.e. the structure and mapping is relatively 1-to-1), but it
is worth noting.
...
We also define a composite, primary key, i.e the `ResultHolder.ResultKey` class, which consists of the `operand` to
the mathematical function along with the `Operator`, which we have defined as an enumerated type.
...
Where the custom `KeyGenerator` was applied in the caching annotations of the service method like so:
...
is used as the identifier in all data access operations against the backend database using the `CalculatorRepository`
(e.g. `calculatorRepository.findById(cacheEntry.getKey())`, in the cache loader's (i.e. _read-through_) case).

If a custom `KeyGenerator` had not been provided, then the "key" would have been the service method parameter only,
and as I already state, the primary key in the database table is a composite key consisting of the operand and operator.
This was deliberate because...

The most fundamental difference between the application domain model and the database model is that while
the application keeps the mathematical calculations in separate, 2 distinct caches (Regions), as seen
...
The database, on the other hand, stores all mathematical calculations in the same table.  That is, both _factorial_
and _square root_ calculations are stored together in the "CALCULATIONS" table.
...
at the operand when performing the cache lookup.  You can't.  You need to know the `operator`, too.

While the individual `CalculatorService` methods for the mathematical functions determine which `operator` is in play,
and even the results of the calculations are kept separately in distinct caches, and therefore, there can only be 1
result per entry (i.e. `operand`) in the individual caches, the database table is not like the cache or the application.
...
One of many reasons for this, especially in _factorial's_ case, is that it is very easy to "overflow" the allowed values
of an `int` type, which is 32 bits.  In fact, with `factorial(13)` you exceed the range of allowed integer values
...
Keep in mind that the following data has been loaded into the backend database already, which is indirectly treated as
"cached" data:
...
For example, if we call `http://localhost:8080/caculator/factorial/4`, you will see the following output:

image::../images/factorial-of-four-before.png[]

The output shows the result of `factorial(4)` was `24`, that the calculation too 3096 milliseconds and the operation
resulted in a _cache miss_.  However, now that we computed the the `factorial(4)`, the value ended up in the "cache"
as well as the backend (embedded, in-memory HSQLDB) database.  So, if we run the operation again, the `latency`
drops to zero (and `cacheMiss` is *false*):
...
the database, i.e. _write-through_) and therefore, the `CaculatorService.factorial(:int)` method is *not* called.
The result, however, is pulled from the cache, not the database.

To see the effects of the `factorial(:int)` method involving the database as part of the inline cache lookup, we can
...
If you hit refresh in your browser now, the application will get the result of `factorial(5)` from the cache
since the result was loaded from the database and put into the cache (_read-through_), and we see that the latency
drops from 12 ms to 0 ms.  However, in both cases, the `cacheMiss` was *false* because the value was found without
invoking the `CalculatorService.factorial(:int)` method:
...
_Inline Caching_ is a powerful caching pattern when you have an external, backend data store that might also be serving
as the application's _System of Record_ (SOR) and you need to keep the cache and database relatively in-sync
with each other.

_Inline Caching_ enables immediate _read-through_ and _write-through_ behavior that keeps the cache
and database consistent.  While the database can serve as a fallback option to priming the cache, the cache
will serve an important role in reducing the contention and load the backend database.

As you have seen in this guide, configuration of _Inline Caching_ is very simple to do with SBDG when using Spring's
Cache Abstraction along with Apache Geode as the caching provider.
