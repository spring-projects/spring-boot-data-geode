...
keeping frequently (read) accessed data in-memory for quick retrieval, which can improve throughput and reduce latency.
...
cloud environment where separate client requests, that are part of the same session, could be routed to different
application instances.  Therefore, application state needs to be maintained independent of the application instances.

NOTE: _Sticky Sessions_ could be used to keep the conversational state associated with a user's Session tied to a single
application instance.  However, using _Sticky Sessions_ is not resilient to failures and as such, are essentially an
anti-pattern in a cloud context.  You should avoid the use of _Stick Sessions_, particularly in a cloud environment,
whenever possible.
...
Rather than scaling-up, you could scale-out by using a sophisticated caching technology that uniformly partitions data
across a cluster of data nodes thereby enabling data access operations to be evenly distributed and intelligently routed
across the cluster.  In addition, data can be replicated for both redundancy and high-availability (HA) purposes making
the cluster more resilient to failure.  Such a data management technology is ideal in a cloud environment.  The cluster
acts as a single, logical unit of pooled resources (Memory, CPU, Disk, and Network) but uses a shared-nothing
architecture.

image::../images/Cluster.png[]

From a Spring Boot application's point-of-view, it is a client in this architecture, and multiple application instances
can access and share the same data.  Indeed, in a Microservices architecture, another application instance must be
prepared to take over in a moments notice if any application instance goes down in order to avoid any perceived
disruption in the users' service.

However, even in a sophisticated, scale-out, client/server architecture such as the one we described above, it still
involves network access, even if only a "single-hop".

So, how might we use caching, to yet again, further reduce resource consumption (e.g. Network) even further?

The key lies in keeping data closer to the point of access, i.e. on the client, in our Spring Boot application.
In essence, we put more responsibility on our Spring Boot applications by increasing the participation of our
applications in this slightly modified architecture in a "_pro-active_" way.
...
the data it is most "_interested_" in.

Additionally, rather than the client having to pull for data changes, the data can be pushed to the client when the data
changes, based on its "_registered interests_".  Therefore, the client only receives the data it is interested in,
in the first place.

Furthermore, the data change events can be conflated so our client application only sees the latest updates, not every
single change that may have occurred due to other application instances accessing the same data.
...
For our example, we will develop a Yellow Pages application with the ability to lookup a person by name to retrieve
their contact information, such as an email address and phone number.
...
.SpringBootApplication for a Geode `CacheServer`
[source,java]
----
...
This class consists of Spring `@Configuration` class to configure the necessary server-side Region (i.e "_YellowPages_")
used to store a person's contact information:
...
ensures that only the latest information (i.e. person contact information) is sent to the clients.  It is possible
that multiple clients maybe accessing and updating a person's contact information.  Therefore, we want to make sure
all the clients have, and are only sent, the latest information, which therefore reduces load on our network.
...
And finally, we include a Spring `@Profile` to enable an embedded Apache Geode Locator and Manager, allowing us to connect
to our Spring Boot, Apache Geode `CacheServer` application using _Gfsh_ (Geode Shell).  Enabling the embedded Locator
and Manager are not necessary when starting the server and running our application.  We will see an example
of this later:
...
the _Look-Aside Caching_ pattern with _Near Caching_ enabled.
...
include some assertions in the Spring Boot `ApplicationRunner` bean to ensure our client configuration is correct.
...
This enables a _local cache_ (a.k.a. "*_Near Cache_*") maintained by Apache Geode on the client,
in our Spring Boot application JVM.

NOTE: The default `ClientRegionShortcut` is `PROXY`, which means there is not local cache.  That is all cache operations
...
to maintain the client's subscription queue created by the interest registration with events matching the regex up to
a specified timeout (configurable on the server).  If the client comes back online before the queue times out,
the events in the queue will be replayed to the client.  If the client does not reconnect before the (server-side)
configured timeout, the queue is discarded.

Durability can be useful for clients that need to receive events for data it missed while the client was away
in the order the events occurred.  Of course, keep in mind that durable client does use up system resources
on the server (e.g. memory).
...
The final boolean parameter, `receiveValues` determines whether the client will receive both keys & values when an
event matching the regex occurs, or will only receive the key for the value that changed.

Receiving only the key 1) minimizes the amount of data sent over the network 2) when the client only wants to
"invalidate" the keyed entry, e.g. by using {apache-geode-javadoc}/org/apache/geode/cache/Region.html#localInvalidate-java.lang.Object-[`Region.localInvalidate(:Object)`].
...
{apache-geode-javadocs}/org/apache/geode/cache/Region.html#registerInterestRegex-java.lang.String-org.apache.geode.cache.InterestResultPolicy-boolean-boolean-[Region.registerInterest(:String, :InterestResultPolicy, :boolean, :boolean)]

There is 1 final bit of configuration on the client-side that we need, and that is to enable subscriptions.
We do so by setting the appropriate Spring Data for Apache Geode (SDG) property
...
We set the `spring.application.name` to help identify the client and additionally set the `server.port` to a unique
value since our Spring Boot application is a Web application.

Now we can discuss the components of the application.

[[geode-samples-caching-near-example-client-side-app-model]]
==== Client-side Application Model

We start by modeling our `Person` and his/her contact information, an email address and phone number:
...
[[geode-samples-caching-near-example-client-side-app-service]]
==== Client-side Application Service

Next, we have our `YellowPagesService` class that implements our Yellow Pages application logic.  This class also uses
...
Technically, our `find(:String)` method should "_idempotent_", but for example purposes, we combine READ with CREATE.
...
[[geode-samples-caching-near-example-client-side-app-controller]]
==== Client-side Application Controller

To make the operations of our Yellow Pages application accessible, we expose a REST-ful API using a Spring Web MVC
...
system and you must enable the "_locator-manager_" profile using `-Dspring.profiles.active=server,locator-manager`.
Additionally, we have enabled the "_server_" profile as well.

If you setup a run profile in your IDE (of choice) and ran the `BootGeodeNearCachingCacheServerApplication` class,
...
Now that the server is running, if you installed Apache Geode locally and set a path to `$GEODE/bin` (where the value of
the `$GEODE` environment variable has been set to the location of your Apache Geode installation), then you can:
...
Now it is time to start 2 instances of the Spring Boot, Apache Geode, `ClientCache` application hosting our Yellow Pages
service.

TIP: Make sure to enable the "_client_" generic profile in addition to 1 of the client-specific profiles,
e.g. "_client-one_", like so: `-Dspring.profiles.active=client,client-one`.  To run a second instance, change
"_client-one_" to "_client-two_".

TIP: Alternatively, rather than using a client instance specific `application.properties` file, you could set the
`spring.application.name` and `server.port` properties using JVM System properties on the command-line, or in your IDE
run configuration profile as so: `-Dspring.application.name=ClientApplicationTwo -Dserver.port=8282`.  Furthermore,
you could set the `server.port` property to the ephemeral port and let the system determine an available port for
the embedded Web Server (i.e. Jetty).  You must need to make note of the port number when the application starts up
so that you can access the Webapp from the browser.  Look for a line containing:
`[info 2019/08/12 13:14:19.755 PDT <main> tid=0x1] Tomcat initialized with port(s): 8181 (http)`.

Once you get both application instances running, you can access the Webapp from your Web browser at the following URL,
`http::/localhost:8181/`:

image::../images/Near-Caching-Example-Webapp.png[]

TIP: To switch between the 2 client app instances, it is useful to have 2 Web browser tabs open
accessing each Web Server port (e.g. `8181` and `8282`).

Next, let's create some data using client app instance by accessing the following URL
...
If you hit the refresh button in our Web browser, then the latency significantly drops (~`0-1|2 ms`) because the value
is being pulled from the "local" cache (i.e. "_Near Cache_), stored on the client.
...
in the log output the for client application instances:
...
To see the effects of updating an entry from a client app instance, let's update "_JonDoe_" from the 2nd client app
...
to the URL, `http://localhost:8181/yellow-pages/JonDoe`, and you should see the updated information:
...
Presto!  You know have just created and experienced a Spring Boot application that is using the _Look-Aside Caching_
pattern enhanced with _Near Caching_.
...
Apache Geode to enable the _Look-Aside Caching_ pattern in our application service methods.  We further enhanced
the caching ability of our application with *_Near Caching_*.

With *_Near Caching_*, we have the ability to further improve on the throughput and latency in our application
as well as make even more efficient use of system resources, since _Near Caching_ gives us:

* A local, client-side cache for quick lookup, transforming our client into efficient, light-weight data containers
for data that is relevant to the client, thereby reducing the load on our servers.
* By _enabling subscriptions_ and _registering interest_, we can have data intelligently pushed to us (rather than
simply pulled when needed) based on only the data the client is specifically interested in, or "subscribed" to.
...
* With 1 more step, it is simple to make the subscription queue on the servers maintained for each client "durable"
so the client goes down, it does not miss any events, if necessary.
